import telegram
from telegram.ext import Application, Updater, CommandHandler, MessageHandler, ContextTypes
...
# Al posto di questa variabile si dovrà utilizzare un file di testo o una tabella
# del database
chatIDs = ["ID_DI_UNA_VOSTRA_CHAT"]
...
async def start(update: Update, context: ContextTypes.DEFAULT_TYPE) -> None:
    ...
    if user_id not in chatIDs:
        chatIDs.append(user_id)

    print(f"Lista delle chat conosciute = {chatIDs}")

    await update.message.reply_text(
        f"Ciao {user.full_name}, inserisci read+data+topic per ottenere i dati!",
    )
...
async def leggi(update, context):
    ...
    try:
        ...
        if results:
            ...
            timestamps = [datetime.strptime(row[4].strftime('%Y-%m-%d') + ' ' + (datetime.min + row[5]).strftime('%H:%M:%S'), '%Y-%m-%d %H:%M:%S') for row in results]
            ...
            if results:
                for row in results:
                    message = f"Temperatura [C]: {row[1]}, Pressione [Pa]: {row[2]}, Luminosità[Lumen]: {row[0]}, Data: {row[4]}, ora: {row[5]}"
...
async def invia_messaggio_telegram(messaggio):
    bot = telegram.Bot(token=TELEGRAM_TOKEN)
    async with bot:
    
        print("Prova invio messaggio bot.")
        
        # La lista delle chat dovrà essere tirata fuori dal file o dal
        # database
        for chat in chatIDs:
            await updater.bot.send_message(chat_id=chat, text=messaggio)
