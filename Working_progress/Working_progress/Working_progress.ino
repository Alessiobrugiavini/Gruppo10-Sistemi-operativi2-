...
const float temperatureHysteresis = 1.0; // Valore dell'isteresi relativa alla temperatura in C
...
const float lightHysteresis = 200.0; // Valore dell'isteresi relativa alla temperatura in C
...
const float pressureHysteresis = 200.0; // Valore dell'isteresi relativa alla temperatura in C
...
void mqttTask(void *parameter) {
  ...
  while (1) {
    ...
    if (xQueueReceive(dataQueue, &data, portMAX_DELAY) == pdPASS) {
      ...
      if (!temperatureThresholdEnabled && data.temperature <= temperatureThresholdHigh - temperatureHysteresis)
      {
      ...
      // Riattiva il monitoraggio della soglia se la temperatura scende sotto la soglia
      if (!pressureThresholdEnabled && data.pressure <= pressureThresholdHigh - pressureHysteresis)
      {
      ...
      {
          sendTelegramMessage("Attenzione: Luce troppo alta[Lumen]!", String(data.light).c_str());
          lightThresholdEnabled = false; // Disattiva il monitoraggio della soglia
      }
      
      // Riattiva il monitoraggio della soglia se la temperatura scende sotto la soglia
      if (!lightThresholdEnabled && data.light <= lightThresholdHigh - lightHysteresis)
      {
